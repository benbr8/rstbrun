...
fn main() {
    ...
    let a = cla.value_of("build");
    let build_dir = match cla.value_of("build") {
        Some(rel_path) => {
            current_dir.join(rel_path).canonicalize().expect("Argument 'build' is not a valid path.")
        },
    ...
    for val in cla.values_of("target").unwrap() {
        ...
        if !path.is_dir() {
            println!("WARNING: {} is not a valid directory. Skipping.", path.to_str().unwrap());
        } else {
